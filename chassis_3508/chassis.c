...
//	}
	...
	double motor_out1 =
		kv*calculate_pid(&PID1, v_target1, motor_chassis[0].speed_rpm, false);
	double motor_out2 =
		kv*calculate_pid(&PID2, v_target2, motor_chassis[1].speed_rpm, false);
	double motor_out3 =
		kv*calculate_pid(&PID3, v_target3, motor_chassis[2].speed_rpm, false);
...
void chassis_stop(void)
{
	// 使用通用PID函数进行停止模式控制
	double motor_out1 =
		calculate_pid(&PID1, 0.0, motor_chassis[0].speed_rpm, true);
	double motor_out2 =
		calculate_pid(&PID2, 0.0, motor_chassis[1].speed_rpm, true);
	double motor_out3 =
		calculate_pid(&PID3, 0.0, motor_chassis[2].speed_rpm, true);
