...
// 使用新INS接口的底盘控制函数
double chassis_motor_1_pid_v2() // 左侧电机
{
	double vx = DBUS_decode_val.rocker[2] * KV;
	double vy = DBUS_decode_val.rocker[3] * KV;
	double vc = DBUS_decode_val.rocker[0] * KV;
	...
	// 直接使用INS结构获取角度
	INS_t* ins = get_INS();
	double sin_yaw = sin(ins->Yaw);
	double cos_yaw = cos(ins->Yaw);
	double vx_set = cos_yaw * vx - sin_yaw * vy;
	double vy_set = sin_yaw * vx + cos_yaw * vy;
	vx = vx_set;
	vy = vy_set;
...
double chassis_motor_2_pid_v2() // 后侧电机
{
	double vx = DBUS_decode_val.rocker[2] * KV;
	double vy = DBUS_decode_val.rocker[3] * KV;
	double vc = DBUS_decode_val.rocker[0] * KV;
...
double chassis_motor_3_pid_v2() // 右侧电机
{
	double vx = DBUS_decode_val.rocker[2] * KV;
	double vy = DBUS_decode_val.rocker[3] * KV;
	double vc = DBUS_decode_val.rocker[0] * KV;
