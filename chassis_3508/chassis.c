							...
							bool use_custom_error, double custom_error, double deadzone)
{
	...
	// 应用死区：当误差绝对值小于死区时，将误差设为0
	if (fabs(pid->cur_error) < deadzone)
	{
		pid->cur_error = 0.0;
		// 在死区内清除积分项，防止积分饱和
		pid->iout = 0.0;
...
{
	return calculate_pid(pid, target_val, current_val, is_stop_mode, false, 0, 0.0);
}

// 包装函数：标准PID计算带死区
static double calculate_pid_standard_with_deadzone(PID_typedef *pid, double target_val, double current_val, bool is_stop_mode, double deadzone)
{
	return calculate_pid(pid, target_val, current_val, is_stop_mode, false, 0, deadzone);
}
...
{
	return calculate_pid(pid, target_val, current_val, is_stop_mode, true, delta_val, 0.0);
}

// 包装函数：位置PID计算带死区
static double calculate_pid_position_with_deadzone(PID_typedef *pid, double target_val, double current_val, double delta_val, bool is_stop_mode, double deadzone)
{
	return calculate_pid(pid, target_val, current_val, is_stop_mode, true, delta_val, deadzone);
}
...
void chassis_control_task(void)
{
	...
	double motor_out1 =
		calculate_pid_standard_with_deadzone(&PID1, v_target1, motor_chassis[0].speed_rpm, false, 10.0);
	double motor_out2 =
		calculate_pid_standard_with_deadzone(&PID2, v_target2, motor_chassis[1].speed_rpm, false, 10.0);
	double motor_out3 =
		calculate_pid_standard_with_deadzone(&PID3, v_target3, motor_chassis[2].speed_rpm, false, 10.0);
...
void chassis_stop(void)
{
	// 使用通用PID函数进行停止模式控制
	double motor_out1 =
		calculate_pid_standard_with_deadzone(&PID1, 0.0, motor_chassis[0].speed_rpm, true, 10.0);
	double motor_out2 =
		calculate_pid_standard_with_deadzone(&PID2, 0.0, motor_chassis[1].speed_rpm, true, 10.0);
	double motor_out3 =
		calculate_pid_standard_with_deadzone(&PID3, 0.0, motor_chassis[2].speed_rpm, true, 10.0);
...
void chassis_control_task2(void) // 定位跑位
{
	...
	position_x_current = -position.world_x;
	position_y_current = -position.world_y;
	position_x_target = position_x_current + 0.0;
	position_y_target = 1000.0;
	position_yaw = get_INS_angle_point()[0];
	...
	beta = position_yaw + 1.5707963 - alpha;
	...
	vx_in_posi = calculate_pid_standard_with_deadzone(&PID_pos_x, /*PossiBuffRcf.X*/ 0.0, -delta_x_equal, false, 10.0); // 前后速度
	vy_in_posi = calculate_pid_standard_with_deadzone(&PID_pos_y, /*PossiBuffRcf.Y*/ 0.0, -delta_y_equal, false, 10.0); // 左右速度

	// double vz_in =  pid calc yAW;// 旋转速度
	// 1. 获取用户输入
	// 遥控器输入，映射到机器人坐标系速度
	double vx_in = vx_in_posi; // 前后速度
	double vy_in = vy_in_posi; // 左右速度
	double vz_in = 0;		   // 旋转速度
